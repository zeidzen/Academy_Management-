...
def Display_Category_Page():
    ...
    display_Category = pages.Display_Category(Id_User)
    return render_template('DB_Category_Table.html', data=display_Category.data)
...
def Delete_Student_Page(id_student):
    if 'Id_User' not in session:
        return redirect(url_for('Home_Page'))
    Id_User = session['Id_User']
    delete_user = pages.Delete_Data(Id_User)
...
def Delete_Product_Page(id_product):
    if 'Id_User' not in session:
        return redirect(url_for('Home_Page'))
    Id_User = session['Id_User']
    delete_item = pages.Delete_Data(Id_User)
    delete_item.Del_product(int(id_product))
...
def Delete_Course_Page(id_course):
    if 'Id_User' not in session:
        return redirect(url_for('Home_Page'))
    Id_User = session['Id_User']
    delete_course = pages.Delete_Data(Id_User)
    delete_course.Del_course(int(id_course))
...
def Delete_Class_Page(id_class):
    if 'Id_User' not in session:
        return redirect(url_for('Home_Page'))
    Id_User = session['Id_User']
    delete_class = pages.Delete_Data(Id_User)
    delete_class.Del_class(int(id_class))
...
def Delete_Category_Page(id_category):
    if 'Id_User' not in session:
        return redirect(url_for('Home_Page'))
    Id_User = session['Id_User']
    delete_category = pages.Delete_Data(Id_User)
    delete_category.Del_category(int(id_category))
    return redirect(url_for('Display_Category_Page'))


@app.route('/delete_post/<id_post>')
def Delete_Post_Page(id_post):
    if 'Id_User' not in session:
        return redirect(url_for('Home_Page'))
    Id_User = session['Id_User']
    delete_post = pages.Delete_Data(Id_User)
    delete_post.Del_post(int(id_post))
    return redirect(url_for('Display_Post_Page'))
...
def update_category():
    data = dict()
    update_category = pages.Update_Data(session['Id_User'])
    if request.method == 'POST':
        data['Id'] = request.form['category_Id']
        data['Name'] = request.form['category_name']

        update_category.update_category(**data)
...
def update_class():
    data = dict()
    update_class = pages.Update_Data(session['Id_User'])
    if request.method == 'POST':
        ...
        data['capacity'] = request.form['capacity']
        update_class.update_classes(**data)
...
if __name__ == '__main__':
    app.run(debug=con.debug, port=con.port, host=con.host_DB)